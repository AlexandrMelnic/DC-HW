...
class Load_Balancer:
	...
	def __init__(self, beta,number_of_servers,number_of_task):
		...
		self.load_balancer=np.zeros(number_of_servers)
	...
	def iterate_one_task_total_processing_time(self):
		min_load_server=np.argmin(self.load_balancer)
		self.task[self.current_task]["server"]=min_load_server
		self.servers[min_load_server]["que"].append(self.current_task)
		self.load_balancer[min_load_server] += self.processing[self.current_task]
		if self.servers[min_load_server]["proces"] == 0:
			item_in_proces=self.servers[min_load_server]["que"].pop(0)
			self.servers[min_load_server]["proces"]=self.processing[item_in_proces]
			self.servers[min_load_server]["processing_task"] = item_in_proces
	...
	def iterate_over_global_time_total_processing_time(self):
		for time in range(self.next_arriving_time):
			self.global_time += 1

			self.load_balancer=np.where(self.load_balancer != 0, self.load_balancer-1 , self.load_balancer)
	...
	def	main(self):
		for i in range(self.number_of_task):
			self.iterate_one_task()
			self.iterate_over_global_time()

		self.finishing_all_the_task()

	def main_total_processing_time(self):
		for i in range(self.number_of_task):
			load_balance.iterate_one_task_total_processing_time()
			load_balance.iterate_over_global_time_total_processing_time()
		self.finishing_all_the_task()

if __name__ == "__main__":

	load_balance=Load_Balancer(beta=40,number_of_servers=20,number_of_task=100000)
	load_balance.main_total_processing_time()
